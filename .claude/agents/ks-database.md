...
프로젝트 루트의 CLAUDE.md에 정의된 데이터베이스 기술 스택을 따른다.
...
2. start_task(task_id, agent_name="database")
3. ARCHITECT의 설계 문서(docs/) 및 프로젝트 CLAUDE.md 참조
4. DB 스키마 설계 + 쿼리 패턴 정의
5. docs/database.md 생성
6. 필요 시 DB 초기화/마이그레이션 코드 구현
...
- SQL 인젝션 방지: 반드시 prepared statement 또는 ORM 파라미터 바인딩 사용
- 마이그레이션은 멱등성 보장
